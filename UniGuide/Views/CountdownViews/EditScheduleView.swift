...
struct EditScheduleView: View {
    ...
    @State private var startDate = Date(timeIntervalSince1970: 0) // 超奇怪只要用到Date()或Date.now，NaviagationLink就會卡bug
    @State private var endDate = Date(timeIntervalSince1970: 0) // 超奇怪只要用到Date()或Date.nowNaviagationLink就會卡bug
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            showEndDatePicker = schedule.endDate != nil
            if showEndDatePicker {
                endDate = schedule.endDate!
            } else {
                endDate = Date()
            }
            isPin = schedule.isPin
        }
...
struct scheduleEditSaveButton: View {
    ...
    var body: some View {
        Button {
            ...
            if !title.isBlank && (endDate > startDate) && showEndDatePicker
                ...
                schedulesManager.updateSchedule(schedule: updatedSchedule)
                dismiss()
            } else {
                if title.isBlank {
